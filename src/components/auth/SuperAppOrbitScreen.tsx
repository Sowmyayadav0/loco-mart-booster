...
export function SuperAppOrbitScreen({ onGetStarted, onSkip }: SuperAppOrbitScreenProps) {
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col justify-between px-4 py-6 sm:py-10 bg-[#FAFDFB] select-none overflow-hidden">
      ...
      {/* TOP HEADER WITH SKIP BUTTON (Exact from Image) */}
      <header className="relative z-10 w-full max-w-md mx-auto flex items-center justify-end shrink-0">
        <button
          type="button"
          onClick={onSkip}
          className="text-xs sm:text-sm font-extrabold text-slate-500 hover:text-slate-900 transition-colors px-3 py-1.5 rounded-xl hover:bg-slate-200/50 cursor-pointer"
        >
          Skip
        </button>
      </header>

      {/* MAIN CONTENT AREA: ROTATING 4 ICONS ORBIT CANVAS + TITLE */}
      <main className="relative z-10 my-auto w-full max-w-md mx-auto flex flex-col items-center justify-center space-y-8">
        {/* ROTATING 4 ICONS ORBIT CONTAINER (Exact from Image) */}
        <div className="relative size-64 sm:size-72 flex items-center justify-center my-2">
          {/* Subtle Glowing Orbit Ring */}
          <div className="absolute size-52 sm:size-60 rounded-full border border-cyan-300/40 bg-cyan-50/20 shadow-inner animate-pulse" />

          {/* Center Brand Badge */}
          <div className="absolute grid place-items-center size-14 rounded-2xl bg-white shadow-lg border border-slate-100/80 z-10 text-cyan-600">
          ...
          >
            {/* TOP ICON: EAT / FOOD (Amber Fork & Knife matching user's image) */}
            <div className="absolute -top-1 left-1/2 -translate-x-1/2">
              <motion.div
                animate={{ rotate: -360 }}
                transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
                className="grid place-items-center size-16 sm:size-18 rounded-3xl bg-white shadow-xl shadow-cyan-900/10 border border-slate-100 text-amber-500 p-3 hover:scale-110 transition-transform cursor-pointer"
              >
            ...
            {/* RIGHT ICON: SHOP (Green Shopping Bag matching user's image) */}
            <div className="absolute top-1/2 -right-1 -translate-y-1/2">
              <motion.div
                animate={{ rotate: -360 }}
                transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
                className="grid place-items-center size-16 sm:size-18 rounded-3xl bg-white shadow-xl shadow-cyan-900/10 border border-slate-100 text-emerald-500 p-3 hover:scale-110 transition-transform cursor-pointer"
              >
                <FiShoppingBag className="size-7 stroke-[2.2]" />
              </motion.div>
            </div>

            {/* BOTTOM ICON: RIDE / CAB (Navy Car Icon matching user's image) */}
            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2">
              <motion.div
                animate={{ rotate: -360 }}
                transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
                className="grid place-items-center size-16 sm:size-18 rounded-3xl bg-white shadow-xl shadow-cyan-900/10 border border-slate-100 text-slate-800 p-3 hover:scale-110 transition-transform cursor-pointer"
              >
                <svg className="size-7" viewBox="0 0 24 24" fill="currentColor">
            ...
            {/* LEFT ICON: SEND / COURIER (Bronze Box Icon matching user's image) */}
            <div className="absolute top-1/2 -left-1 -translate-y-1/2">
              <motion.div
                animate={{ rotate: -360 }}
                transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
                className="grid place-items-center size-16 sm:size-18 rounded-3xl bg-white shadow-xl shadow-cyan-900/10 border border-slate-100 text-amber-700 p-3 hover:scale-110 transition-transform cursor-pointer"
              >
        ...
        {/* TITLE & SUBTITLE (Exact from Image) */}
      ...
      <footer className="relative z-10 w-full max-w-md mx-auto flex flex-col items-center space-y-6 pt-4 shrink-0">
        ...
        {/* GET STARTED PRIMARY CYAN BUTTON (Exact from Image) */}
        <button
          type="button"
          onClick={onGetStarted}
          className="w-full h-13 rounded-2xl bg-gradient-to-r from-cyan-400 via-[#00BCD4] to-teal-600 hover:opacity-95 text-white font-extrabold text-base shadow-lg shadow-cyan-500/30 transition-all flex items-center justify-center gap-2"
        >

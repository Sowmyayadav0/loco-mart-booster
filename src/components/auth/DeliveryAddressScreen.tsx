...
export function DeliveryAddressScreen({ onConfirm, onSkip, onBack }: DeliveryAddressScreenProps) {
  ...
  const ADDRESS_LIST = [
    {
      ...
      icon: FiHome,
      badgeColor: "bg-cyan-100 text-[#044D63]",
    },
    {
      id: "Work",
      title: "Work",
  ...
  async function handleUseCurrentLocation() {
    ...
    setLocLoading(true);
    toast.info("Requesting high-accuracy GPS coordinates...", { id: "gps-status" });
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          if (data && data.address) {
            ...
            toast.success(`Exact GPS Location detected: ${formatted.split(",")[0] || formatted}`, { id: "gps-status" });
          } else {
            ...
            toast.success(`GPS Coordinates detected (${lat.toFixed(4)}, ${lng.toFixed(4)})`, { id: "gps-status" });
          }
        } catch {
          const fallback = `GPS Pin (${lat.toFixed(4)}, ${lng.toFixed(4)}), Bengaluru`;
          setSelectedAddr("GPS Location");
          setCustomAddress(fallback);
          navaStore.setActiveLocation(fallback);
          toast.success("GPS Location detected!", { id: "gps-status" });
        } finally {
          setLocLoading(false);
        }
      },
      (err) => {
        ...
        setCustomAddress(fallback);
        navaStore.setActiveLocation(fallback);
        toast.info("Showing default location. Allow GPS access in browser for live positioning.", { id: "gps-status" });
      },
      {
    ...
    if (cleanPin.length === 6 && /^\d+$/.test(cleanPin)) {
      try {
        ...
        if (Array.isArray(data) && data[0]?.Status === "Success" && data[0]?.PostOffice?.length > 0) {
          ...
          toast.success(`Pincode ${cleanPin} Mapped: ${po.Name}, ${po.District}!`);
        }
      } catch {
        // Fallback
      }
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col justify-between px-4 py-4 sm:py-8 bg-[#FAFDFB] select-none overflow-hidden">
      ...
      {/* TOP BAR WITH BACK BUTTON & SKIP (Exact from Image 2) */}
      <header className="relative z-10 w-full max-w-md mx-auto flex items-center justify-between shrink-0 pt-2">
        <button
          type="button"
          onClick={onBack}
          className="size-10 rounded-full bg-white shadow-md border border-slate-200 flex items-center justify-center text-slate-700 hover:bg-slate-50 transition-colors"
          aria-label="Go Back"
        >
        ...
        <button
          type="button"
          onClick={onSkip}
          className="text-xs sm:text-sm font-extrabold text-[#044D63] hover:underline transition-colors px-3 py-1 cursor-pointer"
        >
          Skip
        </button>
      </header>

      {/* BOTTOM SHEET / CARD CONTAINER (Exact from Image 2) */}
      <main className="relative z-10 my-auto w-full max-w-md mx-auto space-y-4 pt-2">
        <motion.div
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.3 }}
          className="bg-white/95 backdrop-blur-md rounded-3xl p-5 sm:p-6 border border-slate-100 shadow-xl shadow-cyan-900/10 space-y-4"
        >
          {/* SHEET HANDLE PIN */}
          <div className="w-12 h-1 rounded-full bg-slate-200 mx-auto" />
          ...
          {/* SEARCH ADDRESS INPUT (Exact from Image 2) */}
          <div className="relative">
            ...
            <input
              ...
              className="w-full h-12 rounded-2xl border border-slate-200 bg-slate-50/50 pl-10 pr-4 text-xs font-bold text-slate-900 placeholder:text-slate-400 outline-none focus:border-[#00BCD4] focus:bg-white transition-all"
            />
          </div>

          {/* USE CURRENT LOCATION BUTTON (Exact from Image 2) */}
          <button
            type="button"
            disabled={locLoading}
            onClick={handleUseCurrentLocation}
            className="w-full h-12 rounded-2xl bg-gradient-to-r from-cyan-400 via-[#00BCD4] to-cyan-600 hover:opacity-95 text-white font-extrabold text-xs sm:text-sm shadow-md shadow-cyan-500/20 transition-all flex items-center justify-center gap-2"
          >
            <FiCrosshair className="size-4.5" />
            <span>{locLoading ? "Detecting GPS location..." : "Use current location"}</span>
          </button>

          {/* SAVED ADDRESSES SECTION (Exact from Image 2) */}
          <div className="pt-2 space-y-2">
            <div className="flex items-center justify-between text-[11px] font-extrabold tracking-wider text-slate-400 uppercase">
              <span className="flex items-center gap-1 text-[#044D63]">
                ⭐ SAVED ADDRESSES
              </span>
              <span className="text-[#044D63] cursor-pointer hover:underline">VIEW ALL</span>
            </div>

            {/* ADDRESS CARDS LIST (Exact from Image 2) */}
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3.5 p-3.5 rounded-2xl border-2 text-left transition-all ${
                      isSelected
                        ? "border-[#00BCD4] bg-[#EEFBFD] shadow-xs"
      ...
      {/* FOOTER CONFIRM BUTTON (Exact from Image 2) */}
